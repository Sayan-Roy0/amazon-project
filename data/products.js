...
export function getProduct(productId) {
  let matchingProduct; // Storing matching product inside the variable to use its info
...
class Clothing extends Product {
  ...
  constructor(productDetails) {
    super(productDetails); //This will call parent constructor
  ...
  extraInfoHTML() {
    ...
    `; //target="_blank" will open the link on different page
  }
}

/*
const date = new Date(); //It give current date
console.log(date);
console.log(date.toLocaleTimeString());  //it give current time
*/

/*console.log(this); // give undefined
const object2 = {
  a: 2,
  b: this.a,   //Here is no any object to point to so this code will give error
};*/
...
logThis();
logThis.call("hello");  //.call() set a value to the this 
*/

//Load Products from backend and make it to a js object and then into class
export let products = [];

//using fetch is a beter way to make a http request
export function loadProductFetch() {
  const promise = fetch("https://supersimplebackend.dev/products")
    .then((response) => {
      return response.json(); //to get data form the response. it return a promise and we need to wait for this
    })
    .then((productData) => {
      products = productData.map((productDetails) => {
        //here we dont need JSON.parse()
    ...
    .catch((error) => {
      //if there is an error then this catch function will run this for error in case of promises
      //.catch for handle errors in promises
      //this (error) inside the catch parameter contains info about error we can use it if need
      console.log("Unexpected error....!");
      console.log(error);
    });
  //another way to send request to server. by defauilt fetch make a get request
  //when we call fetch it create a promise and we just need to add next step

  return promise;
}

/*
loadProductFetch().then(()=>{   //we can also add a next step here as we return a promise
...
export function loadProducts(fun) {
  ...
  xhr.addEventListener("error", () => {
    //When there is an Error then this function will run otherwise not
    console.log("Unexpected error");
  });
...
export const products = [
  ...
  //.map() runs a function for each element inside the array
  //making the regular object to a class for extra features
